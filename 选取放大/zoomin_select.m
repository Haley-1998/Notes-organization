...
function zoomin_select(init_data, init_cgdata, corarea)
% 根据给定的坐标区域放大选择相应数据
...
switch corarea
    case 'T-Z'
        [x, y] = ginput; % 获取用户点击的坐标
        if ~isempty(x)
            xv = [x' x(1)]; % 创建多边形的x顶点数组
            yv = [y' y(1)]; % 创建多边形的y顶点数组
            hold(gca, 'on');
            plot(gca, xv, yv); % 绘制用户选择的区域
            in1 = inpolygon(init_data(:, 1), init_data(:, 4), xv, yv); % 判断点是否在多边形内
            data2 = init_data(in1, :); % 获取在多边形内的点
    ...
    case 'X-Y'
        ...
        if ~isempty(x)
            ...
            hold(gca, 'on');
            plot(gca, xv, yv); % 绘制用户选择的区域
            in1 = inpolygon(init_data(:, 2), init_data(:, 3), xv, yv); % 判断点是否在多边形内
            data2 = init_data(in1, :); % 获取在多边形内的点
    ...
    case 'X-Z'
        ...
        if ~isempty(x)
            ...
            hold(gca, 'on');
            plot(gca, xv, yv); % 绘制用户选择的区域
            in1 = inpolygon(init_data(:, 2), init_data(:, 4), xv, yv); % 判断点是否在多边形内
            data2 = init_data(in1, :); % 获取在多边形内的点
    ...
    case 'Y-Z'
        ...
        if ~isempty(x)
            ...
            hold(gca, 'on');
            plot(gca, xv, yv); % 绘制用户选择的区域
            in1 = inpolygon(init_data(:, 3), init_data(:, 4), xv, yv); % 判断点是否在多边形内
            data2 = init_data(in1, :); % 获取在多边形内的点
